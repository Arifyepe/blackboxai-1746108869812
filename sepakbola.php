...
$stmt = $conn->prepare("SELECT * FROM products WHERE category = 'sepakbola' ORDER BY type, brand, name");
$stmt->execute();
...
$socks = array_filter($products, fn($p) => $p['type'] == 'socks');
?>
...
<head>
    ...
    <title>Sepakbola - Sports Shop</title>
...
<body class="bg-gray-100">
    ...
    <div class="container mx-auto px-4 py-8">
        <h1 class="font-russo text-4xl text-red-600 mb-8 text-center">Sepakbola</h1>

        <!-- Shoes Section -->
        <section class="mb-12">
            <h2 class="font-russo text-2xl mb-6">Sepatu Sepakbola</h2>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <?php foreach(['Nike', 'Adidas', 'Puma', 'Specs', 'Ortusheight'] as $brand): ?>
                <div class="bg-white rounded-lg shadow-lg overflow-hidden">
                    <img src="https://images.pexels.com/photos/2385477/pexels-photo-2385477.jpeg" 
                         class="w-full h-48 object-cover" alt="<?= $brand ?> Football Shoes">
                    <div class="p-4">
                        <h3 class="font-semibold text-lg mb-2"><?= $brand ?> Football Shoes</h3>
                        <p class="text-gray-600 mb-2">Ukuran: 32-45</p>
                        <p class="text-red-600 font-bold mb-4"><?= formatRupiah(2000000) ?></p>
                        <?php if(isLoggedIn()): ?>
                        <button onclick="location.href='beli.php?id=<?= $brand ?>'" 
        ...
        <!-- Jerseys Section -->
        <section class="mb-12">
            <h2 class="font-russo text-2xl mb-6">Jersey Sepakbola</h2>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <?php
                $teams = [
                    'Indonesia', 'Manchester United', 'AC Milan', 'Real Madrid',
                    'Barcelona', 'AL-NASR', 'PSG', 'Liverpool', 'Persib', 'Persija'
                ];
        ...
        <!-- Socks Section -->
        <section class="mb-12">
            <h2 class="font-russo text-2xl mb-6">Kaos Kaki Sepakbola</h2>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <?php for($i = 1; $i <= 10; $i++): ?>
                <div class="bg-white rounded-lg shadow-lg overflow-hidden">
                    <img src="https://images.pexels.com/photos/6823/team-youth-shirt-soccer.jpg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2" 
                         class="w-full h-48 object-cover" alt="Football Socks">
                    <div class="p-4">
                        <h3 class="font-semibold text-lg mb-2">Kaos Kaki Pro <?= $i ?></h3>
                        <p class="text-gray-600 mb-2">High Quality Football Socks</p>
                        <p class="text-red-600 font-bold mb-4"><?= formatRupiah(150000) ?></p>
                        <?php if(isLoggedIn()): ?>
                        <button onclick="location.href='beli.php?id=sock_<?= $i ?>'" 
                                class="w-full bg-red-600 text-white py-2 rounded hover:bg-red-700 transition">
                            Beli
                        </button>
                        <?php endif; ?>
